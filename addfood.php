...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($conn->query("insert into jelo values(null, '$naziv', $gramaza, $cijena)")){
        echo "<script>
        alert('Uspjesno dodato jelo u ponudu');
        window.location.replace('index.php');
        </script>";
    }else{
        echo "<script>
        alert('Greska u dodavanju jela u ponudu');
        </script>";
    }
}
?>
...
<form method="POST" id="form" action="addfood.php">
  ...
  <div class="form-group">
    ...
    <input type="number" step="0.01" min="0" required class="form-control" name="gramaza">
  </div>
  <div class="form-group">
    <label for="cijena">Cijena u eurima</label>
    <input type="number" step="0.01" min="0" required class="form-control" name="cijena">
  </div>

  <button type="submit" class="btn btn-primary form-control">Dodaj jelo</button>
</form>
</div>
</div>
