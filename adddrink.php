...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($conn->query("insert into pice values(null, '$naziv', $litara, $cijena)")){
        echo "<script>
        alert('Uspjesno dodato pice u ponudu');
        window.location.replace('index.php');
        </script>";
    }else{
        echo "<script>
        alert('Greska u dodavanju pica u ponudu');
        </script>";
    }
}
?>
...
<form method="POST" id="form" action="adddrink.php">
  ...
  <div class="form-group">
    ...
    <input type="number" step="0.01" min="0" required class="form-control" name="litara">
  </div>
  <div class="form-group">
  ...
  <button type="submit" class="btn btn-primary form-control">Dodaj pice</button>
</form>
</div>
</div>
